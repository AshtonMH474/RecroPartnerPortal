...
test.describe('Papers (authenticated)', () => {
    test.beforeEach(async ({ page }) => {
        ...
        const papersBlock = page.locator('[data-testid="PageBlocksPapers"]');
        try {
            await papersBlock.waitFor({ state: 'visible', timeout: 5_000 });
        } catch {
            test.skip(true, 'Papers block not present');
            return;
        }

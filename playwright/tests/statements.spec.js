...
test.describe('Statements (authenticated)', () => {
    test.beforeEach(async ({ page }) => {
        ...
        const statementsBlock = page.locator('[data-testid="PageBlocksStatements"]');
        try {
            await statementsBlock.waitFor({ state: 'visible', timeout: 5_000 });
        } catch {
            test.skip(true, 'Statements block not present');
            return;
        }

...
function ExpandableCard({
    ...
    switch (variant) {
      case 'material':
        return {
          ...
          date: card.lastUpdated ? new Date(card.lastUpdated).toLocaleString("en-US", { month: "short", year: "numeric" }) : "N/A",
        };
      ...
      case 'deal':
        return {
          ...
          tags: card.agency ? card.agency.split(";").map(a => a.trim()).filter(Boolean) : [],
        };
      ...
      case 'ticket':
        ...
        const stage = stageMap[card.properties?.hs_pipeline_stage] || { label: "Unknown", color: "#6b7280" };
  ...
  return (
    <div className="bg-[#1A1A1E] rounded-xl w-[100%] border border-white/15 overflow-hidden transition-all duration-500 ease-in-out">
      <div className={`flex justify-between ${variant === 'ticket' ? 'items-start p-3 md:p-4 gap-4' : ''}`}>
        ...
        <div className={`flex ${variant === 'material' ? 'pl-4 pt-4' : 'gap-x-2 md:gap-x-4'}`}>
          ...
          {/* Title + Description */}
          <div className={`${variant === 'material' ? 'pl-4 w-[100%] relative bottom-1' : 'flex flex-col flex-1'}`}>
            {/* Title Row */}
            <div className={variant === 'ticket' ? 'flex flex-col md:flex-row md:items-center pb-2 md:pb-0 gap-1 md:gap-3' : ''}>
              <h2
              ...
              {variant === 'ticket' && (
                <div className="relative md:w-[200px] w-[150px] h-3 md:h-5 rounded-full bg-gray-800 shadow-inner overflow-hidden">
                  <div
                    ...
                    style={{ width: `${cardData.progressPercent}%`, backgroundColor: cardData.stage.color }}
                  />
              ...
              <p className="text-sm flex flex-wrap items-center gap-1">
                ...
                {cardData.amount && (
                  <span className="text-gray-400">
                    Amount: <span className="text-green-400 font-medium">${Number(cardData.amount).toLocaleString()}</span>
                  </span>
                )}
              </p>
            )}
      ...
      {(variant === 'deal' || variant === 'ticket') && (
        ...
        >
          ...
          {variant === 'ticket' && cardData.description && (
            <p className="text-gray-300 text-sm mt-2 break-words pb-2">

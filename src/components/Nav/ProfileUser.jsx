        ...
        <div className="absolute right-0 mt-2 min-w-40 bg-black border border-white/15 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
          <ul className="text-md">
            {links?.map((link, i) => {
              ...
              const spanProps = {
                className:
                  "relative inline-block after:content-[''] after:absolute after:left-0 after:bottom-0 after:w-0 after:h-[1px] after:bg-[#B55914] hover:after:w-full after:transition-all after:duration-300",
              };
              ...
              if (link?.type == 'logout') {
                return (
                  <li onClick={handleLogout} key={i} {...commonProps}>
                    <span {...spanProps}>{link.label}</span>
                  </li>
                );
              }
              if (link?.type == 'edit profile') {
                return (
                  <li onClick={handleEdit} key={i} {...commonProps}>
                    <span {...spanProps}>{link.label}</span>
                  </li>
                );
              }

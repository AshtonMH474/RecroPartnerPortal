...
function MyDeals({ props }) {
  ...
  const cards = useMemo(() => {
    return active === 'deals' ? deals : tickets;
  ...
  return (
    <div
      ...
      style={{
        minHeight: props?.background == 'black' ? '' : '100dvh',
        background: props?.background == 'black' ? 'black' : '',
      }}
    >
      <div
        className="mt-20 xl:mt-40 max-w-[1400px] mx-auto"
        style={{ paddingTop: props?.background == 'black' ? '50px' : '' }}
      >
      ...
      <div className="flex  flex-col items-center ">
        {!visibleCards.length && (
          <p
            data-tina-field={tinaField(props, 'noDealsText')}
            className=" text-md md:text-lg text-center text-[#C2C2BC] mb-6"
          >
            {props.noDealsText}
          </p>
        )}
